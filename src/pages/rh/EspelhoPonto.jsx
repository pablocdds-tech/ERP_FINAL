...
export default function EspelhoPonto() {
  ...
  const aprovarTudo = async (regs) => {
    for (const r of regs.filter((x) => x.status === "registrado")) await aprovarRegistroPonto(r);
    load();
  };
        ...
        <Table>
          ...
          <TableBody>
            {dias.length === 0 ? (
              ...
              const pendentes = d.registros.filter((r) => r.status === "registrado");
              return (
                  ...
                  <TableCell>
                    <div className="flex flex-wrap gap-1.5">
                      {d.registros.sort((a, b) => a.horario.localeCompare(b.horario)).map((r) => (
                        <span key={r.id} className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded border ${r.status === "rejeitado" ? "border-destructive/40 text-destructive line-through" : r.ajustado ? "border-amber-300 bg-amber-50 text-amber-800" : "border-border"}`}>
                  ...
                  <TableCell>
                    {pendentes.length > 0 && (
                      <div className="flex gap-1">
                        <Button size="sm" variant="outline" className="h-7" onClick={() => aprovarTudo(d.registros)}>
                          <Check className="w-3 h-3 mr-1" />Aprovar dia
                        </Button>
                      </div>
                    )}
                  </TableCell>
