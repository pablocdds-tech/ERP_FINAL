...
export default function Colaboradores() {
  ...
  const load = async () => {
    ...
    setItems(c); setCargos(cg); setLojas(l);
  };
  useEffect(() => { load(); }, []);
  ...
  const filtered = useMemo(() => items.filter((c) => {
    ...
    if (perfilF !== "todos" && c.perfil_pwa !== perfilF) return false;
    if (search && !`${c.nome} ${c.cpf || ""} ${c.email || ""}`.toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }), [items, search, statusF, perfilF]);

  return (
    <PageShell title="Colaboradores" description="Cadastro de pessoas, cargos, vínculos e perfil PWA."
      actions={<Button onClick={() => setDialog({ open: true, mode: "create", record: null })}><Plus className="w-4 h-4 mr-1.5" />Novo</Button>}>
      <Card className="p-4 mb-4">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input className="pl-9" placeholder="Buscar por nome, CPF, email..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
          ...
          <Select value={perfilF} onValueChange={setPerfilF}>
            <SelectTrigger className="w-full md:w-[160px]"><SelectValue /></SelectTrigger>
            <SelectContent>
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader><TableRow className="bg-muted/40">
              <TableHead>Nome</TableHead><TableHead>Cargo</TableHead><TableHead>Loja</TableHead>
              <TableHead>Email</TableHead><TableHead>Perfil</TableHead><TableHead>Status</TableHead><TableHead></TableHead>
            ...
            <TableBody>
              {filtered.length === 0 ? (
                ...
                <TableRow key={c.id} className="hover:bg-muted/30">
                  ...
                  <TableCell><span className="text-xs uppercase">{c.perfil_pwa}</span></TableCell>
                  <TableCell><StatusColaboradorBadge status={c.status} /></TableCell>
                  <TableCell>
