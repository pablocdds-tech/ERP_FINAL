...
export default function ContasPagar() {
  ...
  const load = async () => {
    const [c, l, f] = await Promise.all([
      base44.entities.ContaPagar.list("data_vencimento", 500),
      base44.entities.Loja.list(),
      base44.entities.Fornecedor.list(),
    ]);
    setItems(aplicarVencimento(c)); setLojas(l); setFornecedores(f);
  };
  useEffect(() => { load(); }, []);

  const itemsComNome = useMemo(() => items.map((d) => ({
    ...d,
    fornecedor_nome: fornecedores.find((f) => f.id === d.fornecedor_id)?.nome,
  })), [items, fornecedores]);

  const filtered = useMemo(() => itemsComNome.filter((d) => {
    if (statusFilter !== "todos" && d.status !== statusFilter) return false;
    if (lojaFilter !== "todas" && d.loja_id !== lojaFilter) return false;
    if (search) {
      const s = search.toLowerCase();
      if (![d.descricao, d.fornecedor_nome, d.documento].some((v) => String(v || "").toLowerCase().includes(s))) return false;
    }
    return true;
  }), [itemsComNome, search, statusFilter, lojaFilter]);
  ...
  return (
    <PageShell
      ...
      description="Lançamentos de despesas previstas. Baixe ao efetuar o pagamento."
      actions={
        <Button onClick={() => setDialog({ open: true, mode: "create", record: null })}>
          <Plus className="w-4 h-4 mr-1.5" /> Nova conta a pagar
        </Button>
      }
    >
