...
import { PDV_LIST } from "@/lib/pdv-config";

function CardLink({ item }) {
  const Icon = item.icon;
  const to = item.to || `/admin/pdv/${item.slug}`;
  return (
    <Link to={to}>
...
export default function PdvIndex() {
  return (
    <div>
      <PageHeader
        title="PDV"
        description="Pedidos ao vivo, cozinha (KDS) e integração com o Cardápio Web."
      />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {PDV_LIST.map((c) => <CardLink key={c.slug} item={c} />)}
      </div>
    </div>
  );
}