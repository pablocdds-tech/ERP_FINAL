...
import { IA_LIST } from "@/lib/ia-config";

export default function IAIndex() {
  return (
    <div>
      <PageHeader title="IA e Integrações" description="Agents internos, eventos, webhooks e canais de notificação." />
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {IA_LIST.map((item) => {
          const Icon = item.icon;
          return (
            <Link key={item.tipo} to={`/ia/${item.tipo}`}>
